...
public class CardController {
	...
	public Double priceV;
		...
		@PostMapping
		@Transactional
		public ResponseEntity<CardDto> register(@RequestBody @Valid CardForm form, UriComponentsBuilder uriBuilder) throws IOException {
//			try {
				Card card = form.converter(cardRepository);
				idlangV = card.getIdlang();

				
			    URL urllang = new URL("http://localhost:8080/language/"+idlangV);

			    HttpURLConnection connectionlang = (HttpURLConnection) urllang.openConnection();
				...
				if (idlangV == lang.idlang) {
					...
					URI uri = uriBuilder.path("/card/{id}").buildAndExpand(card.getIdcard()).toUri();
		...
		@GetMapping("/{id}")
		public ResponseEntity<CardDto> detail(@PathVariable("id") Long idcard) {
		...
		@PutMapping("/{id}")
		@Transactional
		public ResponseEntity<CardDto> update(@PathVariable("id") Long idcard, 
				@RequestBody @Valid UpdateCardForm form) throws IOException{
		...
		@DeleteMapping("/{id}")
		@Transactional
		public ResponseEntity<CardDto> delete(@PathVariable("id") Long idcard){
			Optional<Card> optional = cardRepository.findById(idcard);
			if(optional.isPresent()){
